...
const numbersNotMoreThan501 = crazyNumbers.reduce((accumulator, item) => {
  if (item < 501) {
...
console.log(numbersNotMoreThan501);
...
const numbersElevated = numbers.map((item) => {
...
const films2000 = tarantinoMovies.filter((item) => {
...
const namesTvShows = tvShows.reduce((accumulator, item) => {
...
const nameCart = cart.reduce((accumulator, item) => {
  return (accumulator += item.name + "\n");
}, []);
